...
def pokeport_query(name, emotion):
    ...
    transport = AIOHTTPTransport(url = "https://spriteserver.pmdcollab.org/graphql", ssl = False)
...
def main() -> None:
    ...
    for monster in result.get('searchMonster', []):
        ...
        for form in monster.get('forms', []):
            ...
            primary_credit = form.get('portraits', {}).get('creditPrimary', {}).get('name',)
            secondary_credits = [
                credit.get('name') for credit in form.get('portraits', {}).get('creditSecondary', [])
            ]
            ...
            simplified.append({
                ...
                'emotionURL': form.get('portraits', {}).get('emotion', {}).get('url'),
